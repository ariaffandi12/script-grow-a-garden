...
--// ReGui - Ensure this is the correct, updated ReGui library for modern UIs
local ReGui = loadstring(game:HttpGet('https://raw.githubusercontent.com/depthso/Dear-ReGui/refs/heads/main/ReGui.lua'))() -- Assuming this ReGui version supports modern theming
...
local Accent = {
    Primary = Color3.fromRGB(56, 116, 75),  -- A slightly muted green
    Secondary = Color3.fromRGB(80, 150, 100), -- A brighter green for active states
    Background = Color3.fromRGB(30, 30, 30), -- Dark grey background
    Frame = Color3.fromRGB(45, 45, 45),    -- Slightly lighter dark grey for frames
    Text = Color3.fromRGB(230, 230, 230),  -- Light grey for text
    Highlight = Color3.fromRGB(120, 180, 140), -- A subtle highlight green
}
...
ReGui:DefineTheme("ModernGardenTheme", {
	...
	TitleBarBg = Accent.Frame,
	TitleBarBgActive = Accent.Primary,
    ResizeGrab = Accent.Secondary,
    FrameBg = Accent.Frame,
    FrameBgActive = Accent.Primary,
	CollapsingHeaderBg = Accent.Frame,
    ButtonsBg = Accent.Primary,
    CheckMark = Accent.Secondary,
    SliderGrab = Accent.Highlight,
    Text = Accent.Text,
...
local function CreateWindow()
	local Window = ReGui:Window({
		...
		Size = UDim2.fromOffset(350, 250), -- Slightly larger for modern feel
        MinimizeKeybind = Enum.KeyCode.RightControl -- Example keybind for minimizing
	})
	return Window
end
...
local function MakeLoop(Toggle, Func)
	coroutine.wrap(function()
		while task.wait(.01) do -- Use task.wait for modern Roblox practices
...
HarvestNode:Separator({Text="Ignore Variants:"})
...
SellThreshold = SellNode:SliderInt({
    Label = "Crops to sell (threshold)",
    Value = 15,
    Minimum = 1,
    Maximum = 199,
})

--// Auto-Walk & Utility Section
local WalkNode = Window:TreeNode({Title="Auto-Walk & Utility 🚶"})
...
AutoWalkAllowRandom = WalkNode:Checkbox({
	Value = true,
	Label = "Allow random farm points"
})
